#ifndef FSLINALG_VECTOR_SUM_HPP
#define FSLINALG_VECTOR_SUM_HPP
...
template<class Lhs, class Rhs> class VectorSum;

template<class Lhs, class Rhs>
struct VectorTraits< VectorSum<Lhs,Rhs> >
{
...
template<class Lhs, class Rhs> 
class VectorSum : public VectorBase< VectorSum<Lhs, Rhs> >
{
public:
	using Self = VectorSum<Lhs, Rhs>;
	FSLINALG_DEFINE_VECTOR
	
	VectorSum(const VectorBase<Lhs>& lhs, const VectorBase<Rhs>& rhs) : m_lhs(lhs.derived()), m_rhs(rhs.derived()) {}
	...
	void assignToImpl(const Alpha& alpha, VectorBase<Dst>& dst, std::bool_constant<checkAliasing> bc) const requires(IsConvertibleTo<Dst>::value and IsScalar<Alpha>::value)  { m_lhs.assignTo(alpha, dst, bc); m_rhs.increment(alpha, dst, bc); }
	
	template<typename Alpha, class Dst, bool checkAliasing>
	void incrementImpl(const Alpha& alpha, VectorBase<Dst>& dst, std::bool_constant<checkAliasing> bc) const requires(IsConvertibleTo<Dst>::value and IsScalar<Alpha>::value) { m_lhs.increment(alpha, dst, bc); m_rhs.increment(alpha, dst, bc); }
	
	template<typename Alpha, class Dst, bool checkAliasing>
	void decrementImpl(const Alpha& alpha, VectorBase<Dst>& dst, std::bool_constant<checkAliasing> bc) const requires(IsConvertibleTo<Dst>::value and IsScalar<Alpha>::value) { m_lhs.decrement(alpha, dst, bc); m_rhs.decrement(alpha, dst, bc); }
private:
...
template<class Lhs, class Rhs> 
FSLinalg::VectorSum<Lhs,Rhs> operator+(const FSLinalg::VectorBase<Lhs>& lhs, const FSLinalg::VectorBase<Rhs>& rhs) { return FSLinalg::VectorSum<Lhs,Rhs>(lhs, rhs); }

} // namespace FSLinalg


#endif // FSLINALG_VECTOR_SUM_HPP
